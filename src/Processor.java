...
public class Processor {
	...
	static MainMemory mainMemory;// how should we address the memory ?
	int [] functionalUnits = new int[5];//0->add, 1->addD, 2->multiply, 3->load,4->store
	int [] cyclesPerInst = new int [5];
	Object [][] scoreBoard;
	int [] registersUnit = new int [7];
	int pipelineWidth;// ??????
	Object  ROB[][];// ??????
	...
	public Processor() {
		...
		System.out.println("Please enter the access time of the main memory in cycles");
		...
		for (int i = 0; i < cacheLevel; i++) {
			...
			System.out.println("Please enter the writing policy if its a 'write back' or 'write through' in case of HIT");
			String Hit = sc.nextLine();
			System.out.println("Please enter the writing policy if its a 'write back' or 'write through' in case of MISS");
			...
			caches.add(new Cache(size, blockSize, auhto,wpHit, wpMiss, accessTime));
		}
		...
		System.out.println("Please enter how many add double precision funtional units");
		functionalUnits[1] = sc.nextInt();
		System.out.println("Please enter the cost of the addD");
		...
		String instructions= ""; //this variable will hold the instructions inserted by the user
		System.out.println("Please enter you program in the same form as described in the project except that to eliminate all spaces except after the operand, ex.(inst regA,regB,regC). When you are done please enter the letter 'q'");
		while(true){
		...
		System.out.println("Please enter the address where you want to place the program in the memory. P.S. Your range is from word 8174 to word 32768");
		int pointer = sc.nextInt()/2;
		mainMemory = new MainMemory(blockSize, pointer, memAccessTime);
		
		/*
		 * the insertion of instructions in the main memory SHOULD be done here !!!!!!!!
		 */
		System.out.println("If you would like to insert any data initially, please insert the address followed by the value in the form of (address,value), then press enter for a new entry; and when you are finished please enter the character 'q'. If you dont want to insert data enter the character 'q'. P.S. Your range is from word 0 to word 8173");
		String checkData = sc.nextLine();
		while(!checkData.equals("q")){ // insertion of initial data is done here !!!!!
