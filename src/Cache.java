...
public class Cache {
	...
	public Object searchData(int address, int cType) { /* cType = 0 -> DCache, cType = 1 -> ICache*/
		...
		tempIndexLength= (int) (Math.log(bytesSize/Associativity)/Math.log(2));
		tempOffsetLength= (int) (Math.log(BlockSize)/Math.log(2));
		...
		tempOffset= binaryToInteger(binaryValue.substring(tempTagLength+tempIndex, binaryValue.length()));
		tempIndex=binaryToInteger(binaryValue.substring(tempTagLength, tempTagLength+tempIndexLength));
		tempTag=binaryToInteger(binaryValue.substring(0, tempTagLength));

		int pointer = (tempIndex * (BlockSize/2) * Associativity) + tempOffset;
	...
	public void cacheData(int address, int level,int cType, String data) {
		...
		if (data==null){ //no data
			...
			if(cType==0?dValidBit[index]==true:iValidBit[index]==true ){ 
				this.removeToBeReplaced(index, level, cType); //if block was not empty (replacing a block)
			} 
			...
			for(int b=0 ; b <BlockSize/2 ; b++){
				...
				address= binaryToInteger((binaryValue.substring(0, tempTagLength)+binaryValue.substring(tempTagLength, tempTagLength+tempIndexLength)+offset));

				if(Processor.caches.size()>level){ //getting data from next level of caches
					if(cType==0){//dCache
						dData[index+b]=  Processor.caches.get(level).searchData(address, cType);
						dValidBit[index+b]=true;
						dDirtyBit[index+b]=true;
					}else{//iCache
						iData[index+b]=  Processor.caches.get(level).searchData(address, cType);
						iValidBit[index+b]=true;
						iDirtyBit[index+b]=true;
					}
				}else{ //getting data from main memory
					if(cType==0){//dCache
						dData[index+b]=  Processor.mainMemory.data[address];
						dValidBit[index+b]=true;
						dDirtyBit[index+b]=true;
					}else{//iCache
						iData[index+b]=  Processor.mainMemory.data[address];
						iValidBit[index+b]=true;
						iDirtyBit[index+b]=true;
					}
			...
			if (this.searchData(address, cType)==null){ //write miss
				if(cType==0?dValidBit[index]==true:iValidBit[index]==true ){ // replacing existing block 
					...
					if(cType==0){ //dCache
						...
						dDirtyBit[index]=true; // in case of write through the dirty bit array won't affect anything because we don't check on it
				...
				if(wpHit==0) {//write through
					...
					if(Processor.caches.size()>level)
						Processor.caches.get(level).cacheData(address, level+1, cType, data);
					else
	...
	public int FindIndexToCache (int index , int cType){
		...
		return (index * (BlockSize/2) *Associativity) + (  (BlockSize/2)*  ((int)Math.random())  );
	...
	public void removeToBeReplaced (int indexInCacheArray ,int level, int cType){
		...
		if((wpHit==0 )||
				((wpHit==1) &&(cType==0?dDirtyBit[indexInCacheArray]==true : iDirtyBit[indexInCacheArray]==true))){ //if write through or write back & dirty==true
			int indexLength= (int) (Math.log(bytesSize/Associativity)/Math.log(2));
			int offsetLength= (int) (Math.log(BlockSize)/Math.log(2));
			int tagLength= 32- (tempIndexLength+tempOffsetLength);

			String tag = cType==0?Integer.toBinaryString(dTag[indexInCacheArray]):Integer.toBinaryString(iTag[indexInCacheArray]);
			while (tag.length()<tagLength){
				tag = '0'+tag;
			}

			String index = Integer.toBinaryString(indexInCacheArray/(((int)Associativity*(BlockSize/2))));
			...
			for(int b=0 ; b <BlockSize/2 ; b++){
				...
				String data=cType==0? dData[indexInCacheArray+b].toString(): iData[indexInCacheArray+b].toString() ;
